...
public enum StatusEnum {
	...
	;
	private static final Logger LOGGER = LoggerFactory.getLogger(StatusEnum.class);
	...
	public static int getCode(String define){
		try {
			return StatusEnum.valueOf(define).code;
	...
	public static String getMsg(String define){
		try {
			return StatusEnum.valueOf(define).msg;
		} catch (IllegalArgumentException e) {
			LOGGER.error("undefined error code: {}", define);
	...
	public static String getMsg(int code){
		for(StatusEnum err : StatusEnum.values()){
			if(err.code==code){
				return err.msg;
			}
		}
